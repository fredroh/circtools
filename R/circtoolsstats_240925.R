...
circ.plot <- function(x, r = NULL, weight = TRUE, ci = 0.95,
  ...
  stats_mv <- if (!is.null(r) & isTRUE(weight)) {
    circ.stats(angles = x_deg, weights = r, units = "deg", type = type, norm = norm)
  } else {
    circ.stats(angles = x_deg, weights = NULL, units = "deg", type = type, norm = norm)
  }
  ...
  p <- circ.rayleigh(x_deg, r, axial = axial, nperm = 0, seed = NULL)$p.value
...
#' @export
circ.stats <- function(angles, weights = NULL,
                       type = c("angles", "axial"),
                       norm = c("sum", "n"),
                       units = c("deg", "rad")) {
  type <- match.arg(type)
  ...
  keep <- !is.na(angles) & !is.na(weights)
  angles <- angles[keep]
  weights <- weights[keep]
  
  n <- length(angles)
  if (n < 1) stop("At least one angle is required after removing NAs.")
  if (length(weights) != n) stop("Length of weights must match length of angles after NA removal.")
  ...
  if (units == "rad") {
    ...
    radians <- deg2rad(angles) * f
  }
  
  X <- sum(weights * cos(radians))
  Y <- sum(weights * sin(radians))
  
  R <- sqrt(X^2 + Y^2)
  denom <- if (norm == "sum") (sum(weights)^2)/(sum(weights^2))  else n
  R_bar <- R / denom
...
# returns mean in degrees; handles axial doubling internally
circular_mean <- function(x, w = NULL, axial = FALSE, units = "deg", na.rm = TRUE) {
  type <- if (axial) "axial" else "angles"
  stats <- circ.stats(angles = x, weights = w, type = type, norm = "sum", units = units)
  stats$mean_orientation
}

# --- estimate kappa (weighted) ----------------------------------------------
est.kappa <- function(x, w = NULL, bias = FALSE, axial = FALSE) {
...
#’ Compute confidence interval endpoints
#’
#’ @param x       Numeric vector of angles (in degrees)
#’ @param conf.level  Confidence level (e.g. 0.95)
#’ @param w       Optional non‐negative weights
#’ @param axial   Logical; if TRUE treat data as axial (0–180°)
#’ @param na.rm   Logical; if TRUE remove NA pairs
#’ @param method  “clt” or “ori” to pass to confidence.angle()
#’ @return A list with  
#’   * mean:      mean direction (deg)  
#’   * half_width: half‐width of the CI (deg)  
#’   * conf.int:  numeric(2) of lower/upper endpoints
...
confidence.angle <- function(
  ...
  type  <- if (axial) "axial" else "angles"
  stats <- circ.stats(angles = x, weights = w, type = type,
                      norm   = "sum", units = "deg")
...
circ.rayleigh <- function(angles_deg, y = NULL, axial = FALSE, nperm = 0, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  if (!is.numeric(angles_deg)) stop("angles_deg must be numeric (degrees).")
  n <- length(angles_deg)
  if (n < 2) stop("need at least two observations")
  
  nperm <- if (!is.null(y)) 10000 else 0
  ...
  # Case 1: No y provided => standard Rayleigh on working scale
  if (is.null(y)) {
    out$method <- "Rayleigh"
  ...
  # Case 2: y provided => Moore's modified Rayleigh procedure
  if (length(y) != n) stop("angles_deg and y must have same length")
  if (!is.numeric(y)) stop("y must be numeric when provided")
  
  out$method <- "Moore modified Rayleigh"
  df <- data.frame(angle = work_deg, y = as.numeric(y))
  df <- df[order(df$y), , drop = FALSE]
  
  i_idx <- seq_len(n)
  ang_rad_ordered <- df$angle * pi / 180
  sumx <- sum(i_idx * cos(ang_rad_ordered))
  sumy <- sum(i_idx * sin(ang_rad_ordered))
  ...
  # permutation p-value if requested (permute angles relative to y)
  if (!is.numeric(nperm) || length(nperm) != 1 || nperm < 0) stop("nperm must be a nonnegative integer")
  nperm <- as.integer(nperm)
  if (nperm > 0) {
    ...
    for (k in seq_len(nperm)) {
      ...
      sumy_p <- sum(i_idx * sin(ang_rad_p))
      R_p <- sqrt(sumx_p^2 + sumy_p^2)
